...
void memGPIO::createDTS (const memGPIO::gpioPin pin, int pinData, std::string dtsFilename, std::string pinTemplate) {
	...
	printf(">>> Create %s <<<\r\n %s\r\n\r\n",dtsFilename.c_str(),dtsFile.c_str());
...
std::map<std::string, const memGPIO::gpioPin> memGPIO::initializePins () {
	...
	pin->pwm.path="";
	pin->pwm.name="";
	pin->modes.mode0="gpmc_a5";
	pin->modes.mode1="gmii2_txd0";
	pin->modes.mode2="rgmii2_td0";
	pin->modes.mode3="rmii2_txd0";
	pin->modes.mode4="gpmc_a21";
	pin->modes.mode5="pr1_mii1_rxd3";
	pin->modes.mode6="eqep1b_in";
	pin->modes.mode7="gpio1_21";
	pins.insert ( std::pair <std::string, const gpioPin> ( "USR0", *pin ));
	...
	pin->bankId=2;
	pin->eeprom=41;
	pin->mux="gpmc_advn_ale";
	pin->muxRegOffset=0x090;
	pin->ain=0;
	pin->scale=0;
	pin->pwm.module="";
	pin->pwm.sysfs=0;
	...
	pin->modes.mode5="N/A";
	pin->modes.mode6="N/A";
	pin->modes.mode7="mmc1_sdcd";
	pins.insert ( std::pair <std::string, const gpioPin> ( "P8_7", *pin   ) );
	...
	pin->bankId=3;
	pin->eeprom=44;
	pin->mux="gpmc_oen_ren";
	pin->muxRegOffset=0x094;
	pin->ain=0;
	pin->scale=0;
	pin->pwm.module="";
	pin->pwm.sysfs=0;
	...
	pin->modes.mode5="N/A";
	pin->modes.mode6="N/A";
	pin->modes.mode7="gpio2_3";
	pins.insert ( std::pair <std::string, const gpioPin> ( "P8_8", *pin   ) );
